...
const char* PythonPluginInstance::call_tool(const char* name, const char* args_json, MCPError* error) {
    ...
    if (!name || strlen(name) == 0) {
        ...
        if (error) { error->code = -1; error->message = strdup("Invalid tool name"); }
        return nullptr;
    }
    ...
    if (!initialized_) {
        ...
        if (error) { error->code = -1; error->message = strdup("Plugin not initialized"); }
        return nullptr;
    }
    ...
    if (plugin_module_.is_none()) {
        ...
        if (error) { error->code = -1; error->message = strdup("Plugin module not loaded"); }
        return nullptr;
    }
    ...
    try {
        ...
        if (!py::hasattr(plugin_module_, "call_tool")) {
            ...
            if (error) { error->code = -1; error->message = strdup("Python call_tool not found"); }
            return nullptr;
        }
        ...
        if (error) { error->code = -1; error->message = strdup("Unknown error"); }
        return nullptr;
    }
}