...
namespace mcp::transport {
    ...
    Session::Session(asio::ip::tcp::socket socket)
        : socket_(std::move(socket)), session_id_(generate_session_id()) {
    }


    asio::awaitable<void> Session::start(HttpHandler *handler) {
        try {
    ...
    asio::awaitable<void> Session::write(const std::string &message) {
        if (!socket_.is_open()) {
            co_return;
        }
        try {
