...
extern "C" {
    ...
    MCP_API const char* call_tool(const char* name, const char* args_json, MCPError* error) {
        try {
            ...
            py::object result = call_tool_func(std::string(name), std::string(args_json));
    ...
    // Initialize plugin function - this should be called by the plugin system
    // before any other functions
    MCP_API bool initialize_plugin() {
        try {
            // Get the directory of the current DLL
            std::string plugin_dir = "."; // Default to current directory
            
            // Try to initialize Python environment
            if (!initialize_python_plugin(plugin_dir)) {
                std::cerr << "Failed to initialize Python environment" << std::endl;
                return false;
            }
            
            // Try to load the Python plugin module
            if (!load_plugin_module("python_example_plugin")) {
                std::cerr << "Failed to load Python plugin module" << std::endl;
                return false;
            }
    ...
    // Constructor function that is called when the DLL is loaded
    #ifdef _WIN32
    #include <windows.h>
    
    // Use proper Windows API decoration
    BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
        switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH:
            // Initialize the plugin when the DLL is loaded
            if (!initialize_plugin()) {
                // If initialization fails, we could log an error or take other actions
                std::cerr << "Plugin initialization failed during DLL_PROCESS_ATTACH" << std::endl;
                // Note: Returning FALSE would prevent the DLL from loading
                // return FALSE;
            }
            break;
